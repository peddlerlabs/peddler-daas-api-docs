...
|---|:---:|:---:|---|
| ``metaData`` | <span style={{color: 'red'}}>REQUIRED</span> | string | A unique code for you to identify the individual shipment and to avoid duplication of shipments within our system |
...
| ``deliveryAddress.postCode`` | <span style={{color: 'red'}}>REQUIRED</span> | string | A valid Dutch 6PP postcode (<span style={{color: 'green'}}>PREFERRED</span>) or 4PP postcode.<br />6PP postcode example: ``1066EP``<br />4PP postcode example: ``1066`` |
...
| ``estimatedInjectionDate`` | <span style={{color: 'red'}}>REQUIRED</span> | string | [JSON DATE ISO 8601](https://docs.jsonata.org/date-time#json-and-iso-8601) ``2023-07-10T15:43:24.133Z`` estimation when the shipment will be injected to destination distribution hub |
...
| ``carrierServiceLevel`` | <span style={{color: 'green'}}>RECOMMENDED</span> | string | Possible values: ``NEXT_DAY_48`` (DEFAULT), ``SAME_DAY_24``,  ``EXPRESS_0930``, ``EXPRESS_1030`` or ``NEXT_DAY_60`` |
...
Any fields that are not required or recommended should **NOT** be left blank.
...
fetch('https://api-lokl.peddler.com/api/carrier/CARRIER_ID/createShipments', {
  method: 'POST',
  headers: {
