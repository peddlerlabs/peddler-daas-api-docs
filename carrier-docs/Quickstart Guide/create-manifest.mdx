...
| ``deliveryShippingType`` | <span style={{color: 'green'}}>RECOMMENDED</span> | string | ENUM PACKAGE DEFAULT (higher rate), LETTERBOX (lower rate) |
...
| ``carrierOptions`` | <span style={{color: 'green'}}>RECOMMENDED</span> | object | *Only when signature is required. SURCHARGE maybe applicable*<br /><pre lang="json">{<br />"key": "POD",<br />"value": "SIGNATURE"<br />}</pre><br /> *Required when shipping ADR goods*<br /><pre lang="json">{<br />"key": "ADR",<br />"value": "Description of ADR contents"<br />}</pre> |
