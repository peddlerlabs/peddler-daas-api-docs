...
The shipment resource (referred to as an 'instance') is a container for a set of pickup-delivery instructions and their respective attributes. 
Each shipment instance contains one or more pickup-delivery line items, referred to as 'delivery line items'.

In this example we will start with a basic 'shipment instance' and a single 'delivery line item', more complex delivery options are provided in subsequent examples.
...
<!-- Each 'shipment Instance' can have many `deliveryLineItems` ([array] property) which contain instructions for each pickup/delivery pair to take place.
...
In order to create a shipment instance, you will need to make a POST request to the following endpoint:
...
| ``deliveryAddress.address1`` | <span style={{color: 'red'}}>REQUIRED</span> | string |  |
...
| ``deliveryShippingType`` | <span style={{color: 'red'}}>REQUIRED</span> | string | ENUM PACKAGE DEFAULT (higher rate), LETTERBOX (lower rate) |
| ``deliveryContentsType`` | <span style={{color: 'red'}}>REQUIRED</span> | string | Possible values: ``GOODS`` (DEFAULT) ``FOOD``, ``ALCOHOL``, ``BOOKS`` OR ``ADR`` |
| ``estimatedInjectionDate`` | <span style={{color: 'red'}}>REQUIRED</span> | string | JSON DATE "2023-07-10T15:43:24.133Z" estimation when the shipment will be injected to destination country |
| ``carrierOptions`` | <span style={{color: 'red'}}>REQUIRED</span> | object | *Only when signature is required SURCHARGE maybe applicable*<br /><pre lang="json">{<br />"key": "POD",<br />"value": "SIGNATURE"<br />}</pre><br /> *Required when shipping ADR goods*<br /><pre lang="json">{<br />"key": "ADR",<br />"value": "Description of ADR contents"<br />}</pre> |
| ``carrierServiceLevel`` | <span style={{color: 'red'}}>REQUIRED</span> | string | Provided by Peddler "NEXT_DAY_48" |
