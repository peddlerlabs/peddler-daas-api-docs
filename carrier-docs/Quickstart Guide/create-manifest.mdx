...
| ``senderAddress`` | <span style={{color: 'red'}}>REQUIRED</span> | object | Address object containing sender details pertaining to delivery |
...
| ``coliMasterId`` | <span style={{color: 'green'}}>RECOMMENDED</span> | string | A unique ID (if applicable) for the colli which will be used to identify all packages (all collo) constituting the colli. |
