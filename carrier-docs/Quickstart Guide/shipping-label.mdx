...
| ``type`` | <span style={{color: 'red'}}>REQUIRED</span> | string | Type of label.<br />Possble values are ``MANIFEST`` (DEFAULT),  ``ROUTE``, ``RETURN_TO_SENDER``  |
...
fetch('https://api-lokl.peddler.com/api/Carriers/CARRIER_ID/shippingLabel', {
  method: 'GET',
  headers: {
...

{
  ...
  "path": "/api/Carriers/CARRIER_ID/shippingLabel"
}
```
